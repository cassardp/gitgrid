...
const ALLOWED_FIELDS = new Set(['title', 'bio', 'showBio', 'align', 'footer', 'showFooter', 'footerAlign', 'showLanguage', 'showStars', 'showTitle', 'github', 'twitter', 'blog', 'email', 'coffee', 'repos', 'sort']);
...
export async function handlePutConfig(request: Request, env: Env): Promise<Response> {
	...
	for (const key of Object.keys(body)) {
		...
		} else if (key === 'showBio' || key === 'showFooter' || key === 'showLanguage' || key === 'showStars' || key === 'showTitle') {
